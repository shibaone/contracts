...
module.exports = async function(deployer, _, _) {
  ...
  deployer.then(async() => {
    ...
    // Initialize and map BoneToken.
    const boneToken = await MRC20.at('0x0000000000000000000000000000000000001010')
    const maticOwner = await boneToken.owner()
    if (maticOwner === '0x0000000000000000000000000000000000000000') {
      ...
      await boneToken.initialize(ChildChain.address, contractAddresses.root.tokens.BoneToken)
    }
